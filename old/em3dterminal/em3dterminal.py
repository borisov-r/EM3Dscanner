...
class Scanner(object):
    ...
    def __init__(self):
        ...
        #
        # RepRap and Network Analyzer objects created
        #if 'rr' in locals() and 'na' in locals():
        if 'rr' in locals():
            self.start(rr, 'NetworkAnalyzer', log, MAX_XY_AXIS, MAX_Z_AXIS)
        #
        #
        #
        #
    ...
    def start(self, reprap, device, logging,
              MAX_XY_AXIS=2000, MAX_Z_AXIS=1200):
        ...
        dev = device
        #
        # get number of points for the measurement
        points = self.getMeasurementPoints(log, MAX_XY_AXIS, MAX_Z_AXIS)
        #
        # get resolution
        res = self.getResolution(log)
        #
        # get PNA window
        win = self.getPNAwindow(log)
        pass
    ...
    def askForPNAwindow(self, logging):
        ...
        try:
            ...
            if value > 0 and value < 5:
                log.append('PNA window selected: %s' % value)
                return value
            else:
                return True
        except ValueError:
            log.append('Error in askForPoints')
            print "Error please enter positive integer value (1-4)"
            return True

    def askForPoints(self, axis, logging, MAX_XY_AXIS=2000, MAX_Z_AXIS=1200):
        ''' get raw data from terminal and return value
        '''
        log = logging
        try:
            ...
            if axis == 'Z':
                if value > 0 and value < MAX_Z_AXIS:
                    log.append('points for axis %s: %s' % (axis, value))
                    return value
                else:
                    log.append('Error in askForPoints for %s axis' % axis)
    ...
    def askForResolution(self, logging):
        ''' get raw data from terminal and return value
        '''
        log = logging
        try:
            ...
            if (value > 0.09) and (value < 200.0):
                ...
                log.append('scan resolution: %s' % value)
                return value
            else:
                log.append('Error in resolution value')
    ...
    def getPNAwindow(self, logging):
        ''' Get x, y, z points for the measurement.
        '''
        # X axis points
        while True:
            window = self.askForPNAwindow(logging)
            if window is not True:
                break
        #
        return (window)

    def getResolution(self, logging):
        ''' Ask for required resolution in terminal and set it. Returns value.
        '''
        while True:
            resolution = self.askForResolution(logging)
            if resolution is not True:
                break
        #
        return (resolution)

    def getMeasurementPoints(self, log, MAX_XY_AXIS=2000, MAX_Z_AXIS=1200):
        ''' Get x, y, z points for the measurement.
        '''
        # X axis points
        while True:
            xpoints = self.askForPoints("X", log, MAX_XY_AXIS, MAX_Z_AXIS)
            if xpoints is not True:
                break
        # Y axis points
        while True:
            ypoints = self.askForPoints("Y", log, MAX_XY_AXIS, MAX_Z_AXIS)
            if ypoints is not True:
                break
        # Z axis points
        while True:
            zpoints = self.askForPoints("Z", log, MAX_XY_AXIS, MAX_Z_AXIS)
